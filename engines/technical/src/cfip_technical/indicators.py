"""Deterministic, allocation-conscious technical indicator primitives."""
...
def ema(data: Sequence[OHLCV], period: int) -> IndicatorResult:
    """Exponential moving average seeded by the first complete SMA window."""
    _validate_period(period)
    values: list[float | None] = [None] * len(data)
    if len(data) < period:
...
def rsi(data: Sequence[OHLCV], period: int = 14) -> IndicatorResult:
    """Wilder RSI with a first value after ``period`` price changes."""
    _validate_period(period)
    values: list[float | None] = [None] * len(data)
    if len(data) <= period:
...
def atr(data: Sequence[OHLCV], period: int = 14) -> IndicatorResult:
    """Wilder average true range, seeded by the first ``period`` true ranges."""
    _validate_period(period)
    values: list[float | None] = [None] * len(data)
    if len(data) <= period:
...
def bollinger_bands(
    data: Sequence[OHLCV], period: int = 20, deviations: float = 2.0
) -> tuple[IndicatorResult, IndicatorResult, IndicatorResult]:
    """Return middle SMA, upper band and lower band using population deviation."""
    _validate_period(period)
    if deviations < 0:
...
def macd(
    data: Sequence[OHLCV], fast_period: int = 12, slow_period: int = 26, signal_period: int = 9
) -> tuple[IndicatorResult, IndicatorResult, IndicatorResult]:
    """Return MACD line, signal line and histogram with explicit warm-up gaps."""
    ...
    histogram = [
        None if current is None or trigger is None else current - trigger
        for current, trigger in zip(line, signal)
    ]
